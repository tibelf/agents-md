...
export class UniappAdapter extends BaseFrameworkAdapter {
  readonly id = 'uniapp';
  ...
  loadConfig(): FrameworkConfig {
    ...
    const configPath = path.join(rulesDir, 'uniapp/config.json');
  ...
  loadImportantRules(): string[] {
    const rulesDir = getRulesDir();
    const rulesPath = path.join(rulesDir, 'uniapp/important-rules.md');
...
export const uniappAdapter = new UniappAdapter();
